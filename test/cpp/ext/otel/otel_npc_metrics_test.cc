...
class OpenTelemetryIntegrationTestInstrumentDomain final
 ...
 public:
  ...
  static inline const auto kTestUpDownCounter = RegisterUpDownCounter(
      "test.otel_int64_up_down_counter", "A simple int64 up down counter.",
      "unit");
};
...
TEST_F(OpenTelemetryPluginNPCMetricsTest, RecordExportedInt64UpDownCounter) {
  ...
  auto data = ReadCurrentMetricsData(
      [&](const absl::flat_hash_map<
          ...
          std::vector<opentelemetry::sdk::metrics::PointDataAttributes>>&
              data) {
        return !data.contains(OpenTelemetryIntegrationTestInstrumentDomain::
                                  kTestUpDownCounter.name());
      });
  EXPECT_THAT(
      data,
      ::testing::ElementsAre(::testing::Pair(
