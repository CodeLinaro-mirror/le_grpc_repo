...
TEST_F(PartyTest, Test17SpawnedPromises) {
  ...
  bool polled_p16_twice = false;

  party->Spawn(
      "p16",
      [&party, &execution_order, &p17_spawned_from_p16, &p16_part2_waker,
       &p16_part2_polled, &polled_p16_twice]() -> Poll<Empty> {
        if (!polled_p16_twice) {
          polled_p16_twice = true;
          ...
          return Pending{};  // Slot 16 remains occupied while Pending
        }
  ...
  // Explicitly wake up p16 to trigger the second poll
