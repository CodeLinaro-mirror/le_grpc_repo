  ...
  auto success_promise =
      TrySeq(TypeParam::instant_ok(1), [&execution_order](int i) {
        ...
        return TrySeq(TypeParam::instant_ok(i + 1), [&execution_order](int j) {
          // Log the value received from the inner TrySeq's first step
          absl::StrAppend(&execution_order, j);
...
TYPED_TEST(TrySeqTest, HeterogeneousChainWithPending) {
  ...
  decltype(TypeParam::ok(1)) p1 = promise();
  ...
  decltype(TypeParam::ok(1)) p2 = promise();
  ...
  decltype(TypeParam::ok(1)) p3 = promise();
  ...
  decltype(TypeParam::ok(1)) p4 = promise();
