...
TEST(InstrumentIndexTest, RegisterAndFind) {
  instrument_detail::InstrumentIndex& index =
      instrument_detail::InstrumentIndex::Get();
  const auto* description =
      index.Register(nullptr, 0, "test_metric", "Test description", "units",
...
TEST(InstrumentIndexDeathTest, RegisterDuplicate) {
  ...
  index.Register(nullptr, 1, "duplicate_metric", "Desc 1", "units",
                 InstrumentMetadata::CounterShape{});
  EXPECT_DEATH(index.Register(nullptr, 2, "duplicate_metric", "Desc 2",
                              "units", InstrumentMetadata::CounterShape{}),
...
TEST(MetricsQueryTest, LowContentionHistogram) {
...
TEST(MetricsQueryTest, LowContentionGauge) {
  auto storage = LowContentionDomain::GetStorage("example.com");
  std::vector<std::string> label = {"example.com"};
  testing::StrictMock<MockMetricsSink> sink;
...
TEST(MetricsQueryTest, FanOutGauge) {
  auto storage_foo = FanOutDomain::GetStorage("example.com", "foo");
  std::vector<std::string> label_foo = {"example.com", "foo"};
  auto storage_bar = FanOutDomain::GetStorage("example.com", "bar");
  std::vector<std::string> label_bar = {"example.com", "bar"};
