...
class Http2ServerTransportTest : public Http2TransportTest {
 ...
 protected:
  ...
  struct StreamDataBase : public RefCounted<StreamDataBase> {
    ...
    virtual ~StreamDataBase() = default;
  };

  template <typename PromiseFactory>
  struct StreamData : public StreamDataBase {
    explicit StreamData(PromiseFactory&& promise_factory)
        : promise_factory(std::move(promise_factory)) {}

    void StartCall(UnstartedCallHandler unstarted_call_handler) override {
      LOG(INFO) << "StreamData::StartCall";
      auto call_handler = unstarted_call_handler.StartCall();
      call_handler.SpawnGuarded("StreamData",
                                std::move(promise_factory(call_handler)));
    }
  ...
  template <typename PromiseFactory>
  void AddStream(PromiseFactory&& promise_factory) {
    stream_data_ = MakeRefCounted<StreamData<PromiseFactory>>(
...
TEST_F(Http2ServerTransportTest, BasicTest) {
  ExecCtx ctx;
...
//   // This test asserts :
//   // 1. Tests Http2ServerTransport object creation and destruction. The
//   object
