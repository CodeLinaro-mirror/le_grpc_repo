...
class Log {
 ...
 private:
  ...
  struct Fragment {
    ...
    Mutex mu;
    grpc_core::RingBuffer<RecordedEvent, Log::kMaxEventsPerCpu>* active
        ABSL_GUARDED_BY(mu);
    grpc_core::RingBuffer<RecordedEvent, Log::kMaxEventsPerCpu> primary;
    grpc_core::RingBuffer<RecordedEvent, Log::kMaxEventsPerCpu> secondary;
  };
