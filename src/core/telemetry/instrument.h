...
GPR_ATTRIBUTE_NOINLINE auto MakeLabel(LabelNames... label_names) {
  return std::vector<std::string>{absl::StrCat(label_names)...};
}
}  // namespace instrument_detail
...
class InstrumentDomain {
 private:
  class MakeDomainWithLabels {
   public:
    template <typename... LabelNames>
    GPR_ATTRIBUTE_NOINLINE auto operator()(LabelNames... label_names) {
      return new instrument_detail::InstrumentDomainImpl<
          typename Derived::Backend, sizeof...(LabelNames), Derived>(
          instrument_detail::MakeLabel(label_names...));
    }
  };
